...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < rows; ++i) {
        std::cout << "Deallocating row " << i << std::endl;
...
void FillRow(Lista* & row_head, double width, double tile_size) {
    ...
    if (2 * tile_size <= width) {
        Lista* new_tile = new Lista{GIALLO, new Lista{ROSSO, row_head}};
        row_head = new_tile;
        FillRow(new_tile->next->next, width - 2 * tile_size, tile_size);
        return;
    }
}
void CalcolaPavimentoRecur(Pavimento & p, double tile_size, int row, int total_rows) {
    ...
    if (p.larghezza > tile_size) {
        p.area[row] = new Lista{ROSSO, nullptr};
...
void CalcolaPavimento(Pavimento & p, double tile_size) {
    ...
    std::cout << "Allocating " << rows << " rows." << std::endl;
